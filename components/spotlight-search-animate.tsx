    ...
    const searchItems = useMemo<SearchItem[]>(() => {
        ...
        dossiers.forEach((dossier) => {
            ...
            items.push({
                ...
                data: dossier,
                badge: `${statutInfo.label} • ${resteAPayer.toLocaleString()} € restant`,
        ...
        cheques.forEach((cheque) => {
            ...
            items.push({
                ...
                category: "Chèques",
                path: `/cheques/${cheque.id}`,
                data: cheque,
                badge: `${cheque.montant?.toLocaleString() || 0} € ${isValide ? "• Validé" : "• En attente"}`,
                badgeColor: isValide
                ...
                {open && (
                    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
                        ...
                        <motion.div
                            ...
                            className="relative w-[90%] max-w-3xl rounded-2xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl border border-white/20 dark:border-gray-700/50 overflow-hidden"
                        >
                                    ...
                                    {Object.entries(groupedItems).map(([category, items]) => (
                                        <CommandGroup key={category} heading={category} className="px-2 py-2">
                                            {items.map((item) => (
                                                ...
                                                >
                                                    <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700">
                                                        {item.icon}
                                                    </div>
